# TASK: CC-013 ΓÇö Issue Intake Function (Click2Kick Backend)
...
Build `netlify/functions/issue-intake.ts` ΓÇö the backend receiver for Click2Kick
...
1. **Validate payload** ΓÇö reject unknown domains/actions with 400
2. **Thread runId** ΓÇö generate one if not provided (`crypto.randomUUID()`)
3. **No Fake Success** ΓÇö if domain diagnostics aren't available, return `{ success: false, disabled: true }`
4. **Log signal** ΓÇö append to `artifacts/data/issue-intake-log.json` (or Netlify Blobs in production)
5. **Domain mapping:**
   - `storage` ΓåÆ check `NETLIFY_BLOBS_CONTEXT`, return storage mode + health
   - `network` ΓåÆ check AI provider config + social keys configured count
   - `build` ΓåÆ return last build timestamp from `agent-state.json` if available
   - `pipeline` ΓåÆ return `pipeline_mode` from healthcheck data
6. **Webhook secret** (optional) ΓÇö honor `ISSUE_INTAKE_SECRET` env var for external callers
