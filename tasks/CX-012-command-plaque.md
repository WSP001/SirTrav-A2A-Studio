# TASK: CX-012 ΓÇö Command Plaque Mission
...
Build and mount the "Command Plaque" ΓÇö a heraldic System Status Emblem
...
```bash
# 1. PREFLIGHT ΓÇö check task spec + component status
just build-hud

# 2. ORIENT ΓÇö confirm Layer 1-2 gates are clear
just cycle-next-for codex

# 3. BUILD ΓÇö create the component (your work goes here)
# See STEP-BY-STEP below

# 4. VERIFY ΓÇö full truth loop
just mvp-verify

# 5. REPORT ΓÇö generate pulse report
...
**Interactions:**
- Click quadrant ΓåÆ detail panel for that domain
- Click monogram ΓåÆ toggle Inverse Mode (admin auth)
- Loading ΓåÆ skeleton pulse animation
- Error ΓåÆ "offline" indicator (no fake success!)
...
1. **Read spec:** `plans/SIR_TRAVIS_EMBLEM_SPEC.md` (create if missing ΓÇö see handoff)
2. **Create component:** `src/components/SystemStatusEmblem.tsx`
   - Import `THEME` from `../remotion/branding` ΓÇö NEVER hardcode colors
