# TASK: CX-014 ΓÇö The Reality Toggle (Packet Switch Indicator)
...
|-------|-------|-------|---------|
| OFF | Gray (#6B7280) | "Simulation Mode" | Free ΓÇö no real API calls |
| ON | Gold (#D4AF37) | "Real Packet Mode" | Cost Incurred ΓÇö live API |
| ERROR | Red (#EF4444) | "Keys Missing" | 401/503 ΓÇö toggle snaps back |
...
const TOGGLE_COLORS = {
  simulation: '#6B7280',                    // Gray ΓÇö free mode
  real: '#D4AF37',                          // Gold ΓÇö cost incurred
  error: THEME.colors.error,               // #ef4444 ΓÇö keys missing
...
- `configured: true` + `enabled: true` ΓåÆ Gold (Real Mode available)
- `configured: false` ΓåÆ Red snap-back (Keys Missing)
- `enabled: false` ΓåÆ Gray (Simulation Mode, even if keys exist)
...
- `GET /.netlify/functions/healthcheck` ΓåÆ social.twitter.configured/enabled
- `artifacts/council/vault.status.json` ΓåÆ ok, version, timestamp
- Truth Serum summary ΓåÆ last verified timestamp
...
2. Clicking ON checks healthcheck ΓåÆ Gold if keys configured, Red if not
