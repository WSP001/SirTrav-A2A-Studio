# TASK: CC-014 ΓÇö Backend Truth Hardening (X + Integrations + Vault)
...
keys are missing (line 101). This is a "soft lie" ΓÇö HTTP says OK but the body
...
**BEFORE (Soft Lie ΓÇö line 98-111):**
```typescript
...
```typescript
if (!appKey || !appSecret || !accessToken || !accessSecret) {
    console.error("Γ¢ö [X-AGENT] PACKET REJECTED: Missing API keys. No mocks allowed.");
    return {
        statusCode: 503,  // Service Unavailable ΓÇö HONEST status code
        headers,
        body: JSON.stringify({
            success: false,
            disabled: true,
            platform: 'x',
            error: "Γ¢ö PACKET REJECTED: X/Twitter keys not configured. No mocks allowed.",
            required_keys: [
                "TWITTER_API_KEY",
                "TWITTER_API_SECRET",
...
The existing catch block (line 173-210) is already honest ΓÇö it returns real
...
if (!tweetId || tweetId === 'mock-id') {
    throw new Error("Γ¢ö PACKET REJECTED: Real API returned no tweetId. No mocks allowed.");
}
```
...
```typescript
// Helper functions (only active when Vault is initialized ΓÇö fail fast if not)
...
- Missing envs ΓåÆ SKIP with explicit fix hints (not FAIL)
...
1. `just verify-x-real` ΓÇö confirms no mock patterns in publish-x.ts
2. Missing keys ΓåÆ HTTP 503 (not 200)
