#!/usr/bin/env python3
"""
emergency_path_fixer.py — Master Fixer for WM-012 DevKit
=========================================================
...
# ── ATTRIBUTION MAGIC: NDJSON LOGGING ─────────────────────────
...
async def run_agent_analysis(report: dict, hardware: dict):
    ...
    if not HAS_PYDANTIC_AI or not HAS_PYDANTIC:
        print("  ⏭️  AI agent skipped (install: pip install pydantic-ai)")
        return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Emergency Path Fixer — Master Fixer for WM-012 DevKit"
    )
    ...
    print("╔══════════════════════════════════════════════════════════╗")
    print("║  Emergency Path Fixer — Master Fixer (WM-012)           ║")
    print("║  RecursiveLoopInterrupter + HardwareAwareStabilizer      ║")
    print("╚══════════════════════════════════════════════════════════╝")
    print("")
    ...
    hw = detect_hardware()
    print(f"  🖥️  CPU: {hw['CpuModel']} ({hw['CpuBrand']})")
    ...
    if not os.path.exists(target):
        print(f"  ⚠️  Target path not found: {target}")
    ...
    if not violations:
        print("  ✅ No path violations found — directory structure is clean")
        if args.json:
    ...
    # ── Fix ────────────────────────────────────────────────────
    print(f"\n━━━ RecursiveLoopInterrupter — Fixing ━━━")
    t0 = time.time()
    result = collapse_recursion(target)
    fix_ms = int((time.time() - t0) * 1000)

    print(f"\n═══════════════════════════════════════════════════════════")
    print(f"  📦 Files rescued:        {result['files_rescued']}")
    print(f"  🗂️  Empty dirs collapsed: {result['empty_dirs_removed']}")
    print(f"  📏 Max path length:      {result['max_path_len']} chars")
    print(f"  📂 Max depth found:      {result['max_depth']}")
    print(f"  🛡️  PathGuard installed:  {'YES' if result['guard_installed'] else 'NO'}")
    print(f"  📁 Rescue directory:     {result['rescue_dir']}")
    print(f"  ⏱️  Fix time:             {fix_ms}ms")
