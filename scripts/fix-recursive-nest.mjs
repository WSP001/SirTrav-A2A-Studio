#!/usr/bin/env node
/**
 * fix-recursive-nest.mjs ΓÇö Flatten recursive directory loops + prevent reoccurrence
 *
...
// ΓöÇΓöÇ NDJSON LOGGER ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ SCANNER ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ FILE RESCUER ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ PATH GUARD SENTINEL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function writePathGuard(root) {
  ...
  console.log(`  ≡ƒôï Path guard written: ${guardPath}`);
}
...
console.log('Γòæ  fix-recursive-nest ΓÇö Path Loop Destroyer                Γòæ');
...
if (!existsSync(SCAN_ROOT)) {
  console.log(`\nΓ¥î Target path not found: ${SCAN_ROOT}`);
  process.exit(2);
}
...
if (violations.length === 0) {
  console.log('Γ£à No path violations found ΓÇö directory structure is clean');
...
// Report violations
console.log(`\nΓÜá∩╕Å  ${violations.length} violation(s) found:\n`);
for (const v of violations) {
  console.log(`  Γ¥î [${v.reason}] ${v.relPath}`);
...
// Fix
console.log('\n≡ƒöº Rescuing files and flattening recursive structure...');
...
console.log(`≡ƒôª Files rescued:       ${result.rescued}`);
console.log(`≡ƒùé∩╕Å  Empty dirs collapsed: ${result.collapsed}`);
console.log(`≡ƒôü Rescue directory:    ${result.rescueDir}`);
console.log(`ΓÅ▒∩╕Å  Fix time:            ${fixMs}ms`);

if (result.errors.length > 0) {
  console.log(`\nΓÜá∩╕Å  ${result.errors.length} error(s) during rescue:`);
  for (const e of result.errors) {
    console.log(`  Γ¥î ${e.file}: ${e.error}`);
  }
}
...
console.log(`\nΓ£à Recursive nest fixed. Path guard installed.`);
