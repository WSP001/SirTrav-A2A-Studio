...
# Claude Code backend mission brief (lane-bounded, stop-ruled)
claude-backend-mission:
	@echo "🤖 CLAUDE CODE — BACKEND WIRING MISSION"
	@echo "======================================="
	@echo ""
	@echo "WORKSPACE:"
	@echo "  C:/Users/Roberto002/Documents/GitHub/SirTrav-A2A-Studio"
	@echo ""
	@echo "MISSION:"
	@echo "  Review current backend wiring and make only the smallest"
	@echo "  backend-safe change set needed to move the pipeline toward green."
	@echo ""
	@echo "ALLOWED FILE TARGETS:"
	@echo "  netlify/functions/compile-video.ts"
	@echo "  netlify/functions/render-dispatcher.ts"
	@echo "  netlify/functions/render-progress.ts"
	@echo "  netlify/functions/run-pipeline-background.ts"
	@echo "  netlify/functions/generate-attribution.ts"
	@echo "  netlify/functions/lib/cost-manifest.ts"
	@echo "  netlify/functions/lib/quality-gate.ts"
	@echo "  justfile   (only if a missing verification recipe is clearly required)"
	@echo ""
	@echo "READ-ONLY REFERENCE FILES:"
	@echo "  CLAUDE.md"
	@echo "  plans/AGENT_ASSIGNMENTS.md"
	@echo "  plans/AGENT_HANDOFFS_WIRED.md"
	@echo "  docs/RC1_CHECKLIST.md"
	@echo "  MASTER.md"
	@echo ""
	@echo "DO NOT TOUCH:"
	@echo "  src/App.jsx"
	@echo "  src/App.css"
	@echo "  dashboard/component CSS"
	@echo "  Netlify Dashboard settings"
	@echo "  secrets or env vars"
	@echo "  unrelated social publisher files"
	@echo "  unrelated docs"
	@echo ""
	@echo "PRIORITY ORDER:"
	@echo "  1. Confirm current backend state against known blockers"
	@echo "  2. If compile-video.ts still uses local FFmpeg, replace with dispatcher flow"
	@echo "  3. Verify run-pipeline-background.ts preserves runId/projectId and gate order"
	@echo "  4. Verify generate-attribution.ts is real, not placeholder logic"
	@echo "  5. Only add/adjust justfile verification if clearly required"
	@echo ""
	@echo "REQUIRED BACKEND CONDITIONS:"
	@echo "  - No local FFmpeg execution inside Netlify Functions"
	@echo "  - Step 5 uses render-dispatcher pattern"
	@echo "  - Attribution accepts runId and real agent inputs"
	@echo "  - Cost manifest records pipeline steps"
	@echo "  - Quality gate runs before publisher"
	@echo "  - Disabled services return success:false, disabled:true"
	@echo ""
	@echo "STOP RULES:"
	@echo "  STOP if work requires Netlify Dashboard access"
	@echo "  STOP if work depends on missing AWS/Remotion env vars"
	@echo "  STOP if work spills into frontend/UI"
	@echo "  STOP if work requires changing X/Twitter credentials"
	@echo "  STOP if work requires new dependencies"
	@echo "  STOP if repo is already green and change would duplicate landed work"
	@echo ""
	@echo "VALIDATION COMMANDS:"
	@echo "  node scripts/cycle-check.mjs netlify_plugin"
	@echo "  node scripts/cycle-check.mjs healthcheck"
	@echo "  node scripts/cycle-check.mjs no_fake_success"
	@echo "  node scripts/cycle-check.mjs wiring"
	@echo "  npm run build"
	@echo ""
	@echo "OPTIONAL VALIDATION:"
	@echo "  just rc1-verify    (only if already present and valid)"
	@echo ""
	@echo "ACCEPTANCE TESTS:"
	@echo "  - npm run build passes"
	@echo "  - Claude layer gates pass"
	@echo "  - compile-video.ts no longer depends on local FFmpeg"
	@echo "  - pipeline preserves projectId + runId"
	@echo "  - attribution is real, not hardcoded"
	@echo "  - quality gate runs before publisher"
	@echo "  - cost manifest is recorded"
	@echo "  - no fake success introduced"
	@echo ""
	@echo "REPORT FORMAT:"
	@echo "  CHANGED:"
	@echo "    - file / what changed / why"
	@echo ""
	@echo "  VERIFIED:"
	@echo "    - commands run / pass/fail"
	@echo ""
	@echo "  NOT CHANGED ON PURPOSE:"
	@echo "    - Human lane items"
	@echo "    - Codex lane items"
	@echo "    - Antigravity lane items"
	@echo ""
	@echo "  HAND OFF NEXT:"
	@echo "    - Human: Netlify Dashboard build/publish fix + env verification"
	@echo "    - Codex: UI overflow / error boundary"
	@echo "    - Antigravity: practice:test / social dry / RC verification"
	@echo ""
	@echo "COMMIT RULE:"
	@echo "  One commit only if: build passes + Claude gates pass + scope inside allowed files."
	@echo ""
	@echo "SUGGESTED COMMIT TITLES:"
	@echo "  fix(editor): wire render-dispatcher and preserve runId flow"
	@echo "  feat(pipeline): wire attribution, manifest, and quality gate"
