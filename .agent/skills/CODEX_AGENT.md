# Codex Agent ΓÇö Frontend Builder
...
## Session Start ΓÇö LEAN PROTOCOL v3

```bash
just cycle-next-for codex         # 50 tokens ΓÇö ONE line: what to do now
...
Build `src/components/SystemStatusEmblem.tsx` ΓÇö a heraldic 4-quadrant
...
2. **Map** response fields to 4 quadrant colors:
   - Lion (TL) = `storage.healthy` ΓåÆ Gold #D4AF37 or Gray
   - Shield (TR) = `ai.configured && social.*` ΓåÆ Azure #007FFF or Red
   - Cross (BL) = build gate status ΓåÆ Silver #C0C0C0 or Orange
   - Phoenix (BR) = `pipeline_mode` ΓåÆ Ember (THEME.accent) or Dark
...
The emblem is the **User2Agent interface** ΓÇö clicking a quadrant kicks
off the diagnostic view for that domain. This is the "Click2Kick"
pattern applied to system observability:

- Click Lion ΓåÆ Resource Monitor (storage, memory usage)
- Click Shield ΓåÆ Network Status (API endpoints, latency)
- Click Cross ΓåÆ Build/Deploy Status (last deploy, errors)
- Click Phoenix ΓåÆ AI Pipeline Mode (agent statuses, pipeline_mode)
- Click "ST" Monogram ΓåÆ Admin Auth toggle (Inverse Mode)
...
src/remotion/branding.ts         # READ ONLY ΓÇö import tokens, don't modify
```
